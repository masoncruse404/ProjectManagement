"use client"
// - /notes/src/_modals/Projects/EditProject/Desktop/EditProjectDesktop - edit project desktop modal
...
// STYLES
import "./edit-project-desktop.css";
...
import ItContactsEditComponent from "@/app/_components/Modals/Desktop/EditProject/ItContactsEdit/ItContactsEdit";
...
const EditProjectDesktopModal = () => {
    const [error, setError] = useState<string>("")
    const selected_project_context = useSelectedProjectContext()
    const edit_project_modal_state_context = useEditProjectModalStatusContext()
    ...
    function handleModalClose(){ edit_project_modal_state_context.updateValue(false) }
    ...
    async function submitForm(){
        ...
        axiosInstance.put("/v1/project/edit", data).then(response => {
            if(response.status === 200){
                render_context.updateValue(!render_context.value)
                edit_project_modal_state_context.updateValue(false)
    ...
    async function handleProjectDelete(){
        ...
        axiosInstance.delete(`/v1/project/${selected_project_context.value?.Project_ID}/delete`).then(response => {
            if(response.status === 200){
                render_context.updateValue(!render_context.value)
                edit_project_modal_state_context.updateValue(false)
    ...
    if(!edit_project_modal_state_context.value) return;
        ...
        {/* Modal Background Start*/}
            ...
            {/* Wrapper Start*/}
                ...
                <Box className="header">
                    <Box><Typography variant='h6'>Edit Project</Typography></Box>
                    <Tooltip 
                        placement="right"
                        title="Close"
                    >
                    <Box className="modal_new_project_close" 
                        onClick={handleModalClose}>
                        <IconButton sx={{height:"33px", 
                                            width:"33px"
                                        }}>
                            <CloseIcon />
                        </IconButton>
                ...
                <Box className="It-contacts-component"><ItContactsEditComponent></ItContactsEditComponent></Box>
                ...
                <Box className="estimated-time-hours-component"><EstimatedHoursComponent/><CostPerHourComponent/></Box>
                {/* Estimated Time Hours / Cost Per Hour Desktop Container End */}
                {/* Actual Time Hours / Total Cost Desktop Container Start */}
                <Box className="cost-per-hour-component"><ActualTimeHoursDesktopComponent /><TotalCostComponent/></Box>
                ...
                {/* Contact Container Start */}
                    ...
                    {/* Contact Container Grid Start */}
                        ...
                        <Box className="client-contact-component"><ClientContactsComponent numRows={4}/></Box>
                        {/* Client Contacts Container End */}
                        {/* Supplier Contacts Container Start */}
                        <Box className="supplier-contact-component"><SupplierContactsComponent numRows={4}/></Box>
                        {/* Supplier Contacts Container End */}
                        {/* Erp Contact Container Start */}
                        <Box className="erp-contact-component"><ErpContactsComponent numRows={4}/></Box>
                        {/* Erp Contact Container End */}
                    {/* Contact Header End */}
                    </Box>   
                {/* Contact Container End */}
                </Box>
                <Box className="submit-container d-flex ">
...
export default EditProjectDesktopModal